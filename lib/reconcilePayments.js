...
const RECONCILE_LOCK_KEY = 'lock:reconcile_payments';
const RECONCILE_LOCK_TTL_MS = 60000; // 60 seconds

/**
 * Internal executor that fetches pending payments and queries Razorpay.
 */
async function _executeReconciliation() {
 ...
 *
 * Coordinates concurrent executions across instances via a distributed lock.
 *
 * @param {object} [options={}]
 * @param {boolean} [options.bypassLock=false] - If true, bypasses the distributed lock.
 * @returns {Promise<{checked: number, activated: number, stillPending: number, errors: Array<{orderId: string, message: string}>, skipped?: boolean, reason?: string}>}
 */
async function reconcilePendingPayments({ bypassLock = false } = {}) {
  if (!bypassLock) {
    const lock = await acquireLock(RECONCILE_LOCK_KEY, RECONCILE_LOCK_TTL_MS);
    if (!lock.acquired) {
      console.log('[reconcile] Reconciliation already in progress on another instance. Skipping.');
      return { checked: 0, activated: 0, stillPending: 0, errors: [], skipped: true, reason: 'locked' };
    }
    try {
      ...
      try {
        ...
        console.error('[reconcile] Failed to release lock:', releaseErr);
      }
    }
  }
