...
const BATCH_LIMIT = 500; // generous; re-run picks up any remainder next tick
...
async function runCommitModeMidnightSweep() {
  ...
  // Every user currently on Commit Mode — the population this sweep
  // needs to judge for yesterday, whether or not they have a progress
  // row yet.
  ...
  if (!activeUsers || activeUsers.length === 0) {
    return { checked: 0, terminated: 0, kept: 0, todayIst, yesterdayIst };
  }
  ...
  return { checked: activeUsers.length, terminated, kept, todayIst, yesterdayIst };
}
