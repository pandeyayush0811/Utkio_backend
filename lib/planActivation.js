...
const PLAN_PRICES_PAISE = {
  starter: 9900,     // ₹99
  commit_mode: 12100 // ₹121 — Starter's full feature set + the daily
                      // accountability contract, see lib/commitMode.js
};
...
async function activatePlan({ payment, razorpay_payment_id }) {
  ...
  // Clear any stale Commit Mode termination flags so re-purchasing or renewing
  // starts fresh with an active, un-terminated status.
  if (updatedPayment.plan === 'commit_mode') {
    updatePayload.commit_mode_terminated_at = null;
    updatePayload.commit_mode_termination_reason = null;
  }
